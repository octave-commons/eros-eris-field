...
async function vexxSimilarityMatrix(candidates: SemanticEmbeddingNode[], peers: SemanticEmbeddingNode[], vexx: VexxCosineConfig): Promise<SimilarityRow[][] | null> {
  ...
  const maxCandidatesPerCall = Math.max(1, Math.floor(vexx.maxCandidatesPerCall ?? 256)) || 256;
  const optimalBatchSize = Math.max(1, Math.floor(vexx.optimalBatchSize ?? 128)) || 128;
