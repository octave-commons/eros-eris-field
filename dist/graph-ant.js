export class GraphAntSystem {
    ...
    stepAnt(ant) {
        ...
        this.pheromone.set(key, Math.min(this.config.maxPheromone, (this.pheromone.get(key) ?? 0.5) + this.config.depositRate * novelty));
    ...
    stats() {
        let sum = 0;
        for (const ph of this.pheromone.values())
            sum += ph;
        return {
